...
const fetchSoap = async (url, options) => {
  ...
  if (status !== 200) {
    const errorDetails = xml
      .querySelector("errorstring")
      .textContent.replace(/[\n\r]+|[\s]{2,}/g, " ")
      .trim()
    // Throw FETCH Type Error
    throw new Error(
      `${errorDetails || "Error making SOAP Request"} (${status})`
    )
  }
...
const processSiteUrl = (siteUrl, webService) =>
  `${siteUrl.replace(/\/$/, "")}/_vti_bin/${webService}.asmx`
...
const processUpdates = (batchCmd, items) => {
  const mapToFields = item => {
    return Object.keys(item)
      .map(
        staticName =>
          `<Field Name="${staticName}">${escapeColumnValue(
            item[staticName]
          )}</Field>`
      )
      .join("")
